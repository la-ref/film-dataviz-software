...
class FilmView (QWidget):
    ...
    # signaux
    next : pyqtSignal = pyqtSignal(int)
    previous : pyqtSignal = pyqtSignal(int)
    ...
    # methods
    def update(self : Self, record : dict, genres : list[str], distributors : list[str],licenses : list[str], index, taille) -> None:
        ...
        self.msg.setText(str(index+1) + "/" + str(taille+1))
    ...
    # callbacks
    def cbNext(self: Self) -> None:     self.next.emit(self.__currentIndex)
    def cbPrevious(self: Self) -> None: self.previous.emit(self.__currentIndex)
...
if __name__ == "__main__" :
    ...
    db.loadCSV()
    fv = FilmView({})
    def up(currentIndex : int):
        if currentIndex == len(db.db)-1:
            currentIndex = 0
        else:
            currentIndex = currentIndex + 1
        print(currentIndex)
        fv.update(db.db[currentIndex],db.genres, db.distributors, db.licences, currentIndex, len(db.db)-1)
    
    def down(currentIndex : int):
        if currentIndex == 0:
            currentIndex = len(db.db)-1
        else:
            currentIndex = currentIndex - 1
        print(currentIndex)
        fv.update(db.db[currentIndex],db.genres, db.distributors, db.licences, currentIndex, len(db.db)-1)
    
    fv.update(db.db[fv.getIndex()],db.genres, db.distributors, db.licences, fv.getIndex(), len(db.db)-1)
    fv.next.connect(up)
    fv.previous.connect(down)
    print(len(db.db)-1)
